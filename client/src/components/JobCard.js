...
function JobCard({job}) {
  ...
  const [isSaved, setIsSaved] = useState(false);
    /* 
    {
    "gitHubJobsId": "421871bb-05d3-4fbe-b00c-3f372fa35584",
    "type": "Full Time",
    ...
    */
    ...
    const handleSaveJobClick = () =>  {
      ...
      API.savejob(newFavorite).then((res)=>{
        setIsSaved(true);
    ...
    console.log(favoriteId)
     API.delete(favoriteId)
    // .then((res) => loadFavorite())
    //     .catch((err) => console.log(err));
    }
    ...
    <Card>
      <Card.Body> 
        ...
        <ul>
          ...
          <br />
          <button disabled={isSaved} type="button"  onClick={() => handleSaveJobClick()} href="profile" className="btn btn-info">Save</button> {""}
